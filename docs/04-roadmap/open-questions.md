...
- 🟢 [ADR-0010](../05-decisions/0010-web-frontend-hosting.md) — Web frontend hosting (SvelteKit SSR on `mac-mini-2` via `@sveltejs/adapter-node` on Bun; one canonical origin per environment — `i.dinkerwupp.com` prod, `dev.dinkerwupp.com` dev — with HTTPS via DNS-01 ACME; Phoenix owns `/api/*` and `/socket`; reverse-proxy software, session/cookie mechanism, and dev/prod data isolation deferred inside the ADR with triggers).
...
- **Reverse-proxy software** for the `mac-mini-2` front door (Caddy / nginx / Traefik / other). Must support DNS-01 ACME and HTTP/1.1 upgrade for `/socket`. Deferred inside [ADR-0010](../05-decisions/0010-web-frontend-hosting.md); trigger to close is before the Pre-Phase 0 spike serves a SvelteKit page through the front door.
